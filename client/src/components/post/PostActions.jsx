...
function PostActions({ post }) {
  ...
  const { feed: posts } = useSelector((state) => state?.post);
  ...
  const handleLikeAndUnlike = async () => {
    ...
    try {
      const response = await dispatch(likeUnlikePost(post._id));
  ...
  return (
    <div className="flex flex-col">
      <div className="flex gap-3 my-2" onClick={(e) => e.preventDefault()}>
        ...
        <SavepostSVG postId={post._id} theme={theme} />
...
// Component for Savepost the thread
const SavepostSVG = ({ theme, postId }) => {
  const dispatch = useDispatch();

  const handleSavepost = async () => {
    const response = await dispatch(Savepost(postId));
